#!/usr/bin/env node
// Modelmeter — historical event backfill for any provider.
//
// Discovers significant provider announcements via Hacker News (Algolia API),
...
//   BACKFILL_PROVIDER     provider id slug, e.g. "anthropic" (must be in PROVIDER_ENUM)
//   BACKFILL_DOMAIN       HN URL domain filter, e.g. "anthropic.com"
//
// Env optional:
//   BACKFILL_FROM         YYYY-MM-DD, default 3 years ago
//   BACKFILL_TO           YYYY-MM-DD, default today
//   BACKFILL_MIN_POINTS   HN points threshold, default 100
...
const PROVIDER = process.env.BACKFILL_PROVIDER;
const DOMAIN = process.env.BACKFILL_DOMAIN;
...
if (!PROVIDER || !PROVIDER_ENUM.has(PROVIDER)) {
  console.error(`BACKFILL_PROVIDER missing or not in enum. Got: ${PROVIDER}`);
  console.error(`Valid values: ${[...PROVIDER_ENUM].join(", ")}`);
  process.exit(2);
}
if (!DOMAIN) {
  console.error("BACKFILL_DOMAIN missing. E.g. BACKFILL_DOMAIN=anthropic.com");
  process.exit(2);
}

console.log(`Backfilling ${PROVIDER} events (domain: ${DOMAIN}) ${FROM} → ${TO} (min HN points ${MIN_POINTS}, score min ${SCORE_MIN})…`);

const hits = await fetchHnHits();
console.log(`HN Algolia returned ${hits.length} story hit(s) on ${DOMAIN}.`);
...
const branch = `backfill/${PROVIDER}-${FROM}-to-${TO}`;
sh(`git checkout -B ${branch}`);
sh(`git add events/current.json`);
sh(`git commit -m "events: ${PROVIDER} backfill ${FROM} to ${TO} (${fresh.length} entries)"`);
...
if (prExistsRaw) {
  ...
  const bodyPath = `/tmp/backfill-${PROVIDER}-${FROM}-${TO}.md`;
  writeFileSync(bodyPath, body);
  sh(`gh pr create --title "events: ${PROVIDER} backfill ${FROM} to ${TO}" --body-file ${bodyPath} --base main --head ${branch}`);
}
...
async function fetchHnHits() {
  ...
  const domainPattern = new RegExp(`\\/\\/(www\\.)?${escapeRegex(DOMAIN)}\\/`);
  const all = [];
  let page = 0;
  while (page < 20) {
    const url = `https://hn.algolia.com/api/v1/search_by_date?tags=story&numericFilters=created_at_i>${fromTs},created_at_i<${toTs},points>=${MIN_POINTS}&query=${encodeURIComponent(DOMAIN)}&restrictSearchableAttributes=url&hitsPerPage=100&page=${page}`;
    ...
    const hits = (data.hits ?? []).filter((h) => h.url && domainPattern.test(h.url));
    all.push(...hits);
...
function normalizeProviders(arr) {
  if (!Array.isArray(arr)) return [PROVIDER];
  const filtered = arr.map((s) => String(s).toLowerCase().trim()).filter((s) => PROVIDER_ENUM.has(s));
  const unique = Array.from(new Set(filtered));
  if (!unique.includes(PROVIDER)) unique.unshift(PROVIDER);
  return unique.length ? unique : [PROVIDER];
}
...
function buildPrBody(events, from, to) {
  const lines = [
    `Auto-drafted backfill of ${PROVIDER} events from **${from}** to **${to}**.`,
    ``,
    `Source: Hacker News Algolia API → ${DOMAIN} links with ≥${MIN_POINTS} points → Venice-scored ≥${SCORE_MIN}/10.`,
    ``,
